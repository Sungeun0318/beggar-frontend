...
- 방 상태에서 받은 `roomNo`, 지역, 추천 태그 목록을 사용해 추천 API를 호출한다.
- 추천 태그는 반장이 선택한 방 태그만 사용한다. 태그가 1개면 칩 영역을 숨기고, 여러 개면 선택된 태그들만 보여준다.
- 태그 버튼을 누르면 같은 방/지역 기준으로 추천 API를 다시 호출한다.
- 위치 변경에서 현재 위치 또는 카카오 로컬 검색 결과를 선택하면 좌표/반경 기준으로 추천을 다시 호출한다.
- 추천 카드는 업소명, 업종, 대표 메뉴명, 가격, 주소, 카테고리 기본 이미지, 우상단 착한가격업소 띠를 표시한다.
- 추천 카드를 누르면 카카오맵 이동 확인창이 뜨고, 확인 시 백엔드 응답의 `mapUrl`을 외부 앱/브라우저에서 연다.
- 백엔드가 내려준 1인 추천 예산 안내와 fallback 문구를 함께 보여준다. 추천이 부족하면 “조건에 맞는 추천이 부족해서 범위를 넓혀봤어요.” 톤으로 안내한다.
- 거지방 내부 화면으로 진입.
...
| 위치 변경 | 현재 위치 또는 검색 위치를 선택하고 추천 API 재호출 |
| 추천 카드 | 카카오맵 이동 확인창 표시 후 `mapUrl` 열기 |
| 안내 카드 화살표 | 미연결 |
...
3. 다음 기능: 영수증 등록 시 착한가격업소 매칭 후 방별 거지력 점수에 인증 방문 수 반영.
